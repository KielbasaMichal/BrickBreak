...
void top_screen()
{
	...
	while (States == Top)
	{
		...
		while (windows.pollEvent(event))
		{
			...
			{
				if (event.mouseWheel.delta == 1)		//rolka do g�ry
				{
					pos_y -= 50;
					TView.reset(sf::FloatRect(0, pos_y, WIDTH, HEIGHT));
				}
		...
		windows.setView(windows.getDefaultView());		//Koniec element�w dla nowego widoku, rysujemy elementy na kt�re widok nie ma wp�ywu

		windows.draw(back);
		windows.draw(down);		//Do przesuwania widoku w d�
		windows.draw(up);		//Do przesuwania widoku w g�r�
